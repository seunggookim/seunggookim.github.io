...
The ridge projection matrix $$\mathbf P = \mathbf{X}_2 \mathbf{X}_1^{+} + \lambda \mathbf{I}$$ can be somewhere between these two cases depending on the optimal $$\lambda$$:

$$
\begin{cases}
\lambda \mathbf{I} & \text{if } \lambda \gg 0 \\
\mathbf{X}_2 \mathbf{X}_1^{+} & \text{if } \lambda = 0
\end{cases}
$$
