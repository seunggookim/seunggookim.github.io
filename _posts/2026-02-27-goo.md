...
The GOO algorithm is a joke because it is an unnecessarily complicated solution to a simple problem. Its name itself is also a joke because it is actually a backronym of the current official romanisation of the second syllable of my first name (구=九=Goo). Finally, it is a joke because it is more inefficient than an exhaustive search (due to the duplicate randomisations). However, it is not a joke when the random search actually finds the optimal solution much faster than the exhaustive search. This depends on the dimensions of the problem and the arbitrary number of iterations chosen for the random search.
