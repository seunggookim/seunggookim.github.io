...
The Assignment matrix A also has policy-wise constraint:
...
where $H(x-1)$ is the Heaviside step function with a threshold of 1 and $||\cdot||_1$ denotes L1 norm. 
...
$$
U = ζ(\mathbf{Z}, \mathbf{d}) = \sum_{i,j} z(i,j).d(i),
$$
...
[HMM!]
<!-- $$
V = χ(\mathbf{A}, \mathbf{d}) =  \sum_i H(||\mathbf{a}_i||_1-1).d(i),
$$ -->
