...
It is important to note that, in the musical context, both absolute and relative pitch refer to the recognition of pitch chroma.
That is, simply knowing which note is higher than the other does _NOT_ mean relative pitch in the musical context. Being able to tell their intervals (e.g., a major second), or a solfegio with respect to the given reference (e.g. "Sol") is the relative perception of pitch chroma.
...
In {% cite kim2017fn %}
, we proposed to distinguish the former as the absolute memory of pitch (i.e., without separation of height and chroma) and the latter as the absolute perception of pitch chroma.
...
In traditional music education, students write down a short excerpt (e.g., 8 bars) of music, from a tonal melody to a atonal four-voice chroale.
The excerpts are confidentially, arbitrarily created by teachers.
...
- **response methods**: pen & paper, voice & microphone, clicking on a displayed keybaord figure, radio-button-like user-interfaces, press a key on a muted digital keyboard
- **tone timbre**: sine wave, digial piano rendering, actual piano recording, other instruments?
- **octave ranges**: two to five
  (It is important to note that the tones _MUST_ be presented over at least two octaves, otherwise the classifications of pitch-chroma and pitch-height are inseparablly confounded.)
...
Also interestingly, many other fundamental perception do not seem to differ between musicians with and without absolute pitch. For example, frequency discrimination, gap detection (temporal resolution), and interaural time difference discrimination (i.e., spatial resolution) were found indifferent
...
However, as opposed to the claim, the training focuses on developing relative pitch and working memory, which in combination can allow musicians for performing well in traditional absolute pitch test where the reference note is given at the beginning of the test.
Thus, it seems likely that the acquiable skill in adulthoods is the relative recognition (i.e., calculation of intervals) of pitch chroma, rather than the absolute perception of pitch chroma.

Recently, however, a group of scientists has been suggested that absolute pitch can be acquired in adulthoods.
...
This suggests <b>an overemphasis on developing absolute pitch can harm a more releveant musical skill, relative pitch</b>.
...
One might imagine the preservation of the absolute frequency information from the peripherals (i.e., knowing which hair cell fires; or even which inferior collicolus neuron fires such as in [(Ross et al., 2005)](https://doi.org/10.1016/j.yebeh.2005.05.019)).
...
Rather, in totality of observations, it seems that there exist a cortical processing that invovles the computational requirements (2) and (3).
...
the music and the arts are lots more complicated (even in the classical sense) than just the keen perception.
